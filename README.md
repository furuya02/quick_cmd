...
通常インストール（`pip install .`）でインストールした場合、コマンドファイルを削除しても site-packages に古いファイルが残っていることがあります。

以下のコマンドで解決できます：

```bash
# パッケージを再インストール
pip uninstall quick_cmd -y
pip install -e .
```

開発モード（`pip install -e .`）でインストールすると、ローカルの変更が即座に反映されるため、この問題は発生しません。
